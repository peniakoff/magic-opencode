...
require_trusted_wrapper() {
  local relative_path=".cursor/scripts/parallel-worktrees.sh"
...
slot_worktree() {
  printf '%s/.cursor/worktrees/%s\n' "$REPOSITORY_ROOT" "$1"
}

relative_worktree() {
  printf '.cursor/worktrees/%s\n' "$1"
}
...
validate_scope_path() {
  ...
  case "/$path" in
    /.git | /.git/* | /.cursor/worktrees | /.cursor/worktrees/* | \
...
create_lane() {
  ...
  require_clean_root
  git check-ignore -q .cursor/worktrees/probe ||
    die ".cursor/worktrees/ must be ignored by Git before using parallel lanes"

  local state worktree scope_tmp other other_state path normalized
  state="$(slot_state_dir "$slot")"
  worktree="$(slot_worktree "$slot")"
  [[ ! -e "$state" && ! -e "$worktree" ]] || die "slot '$slot' already exists"
  scope_tmp="$(mktemp "${TMPDIR:-/tmp}/cursor-scope.XXXXXX")"
...
inspect_lane() {
  ...
  state="$(slot_state_dir "$slot")"
  worktree="$(slot_worktree "$slot")"
  artifact="$(mktemp "${TMPDIR:-/tmp}/cursor-lane-inspect.XXXXXX")"
...
integrate_lane() {
  ...
  patch="$(mktemp "${TMPDIR:-/tmp}/cursor-lane-patch.XXXXXX")"
