...
4. Add dependencies only when required. Choose the latest stable version compatible with the repository runtime and lockfile, verifying unfamiliar APIs with Context7 or primary vendor documentation. Before any other feature edit, the sole implementer must update registry dependencies on the clean feature branch through `bash .opencode/scripts/dependency-update.sh <add|add-dev|remove> <npm|yarn|pnpm|bun> <package>...`, or use its `batch <manager> <operation>:<package>...` form for a mixed runtime, development, and removal transaction. The wrapper validates identifiers, fixes dependency class internally, disables lifecycle scripts, rejects non-metadata output, and refuses a dirty tree. Immediately inspect and preliminarily review the resulting manifest and lockfile before continuing implementation. Do not perform unrelated upgrades. If a required generator or migration is not exposed as an already reviewed and allowed repository script, stop and request explicit user action or a project-specific permission instead of inventing a command.
...
4. Inspect PR mergeability, review decision, and status checks. Run the wrapper's `wait-checks <pr-number>` action. It observes the complete 60-second registration window so delayed workflows can appear, watches all registered checks to completion, and then requires a 30-second stable terminal check set, restarting the watch whenever another check appears. Internally, `wait-checks` uses `gh pr checks --watch` with the 60-second registration and 30-second stabilization windows. Persistent zero checks, an unstable set, a pending required review, requested changes, a conflict, or a skipped/cancelled/failed check blocks merge.
...
6. When the PR is mergeable, review is complete, and all checks pass, read and record the reviewed `headRefOid`, then run `bash .opencode/scripts/github-delivery.sh merge <pr-number> <head-sha>`. The merge action independently repeats the complete registration, watch, and stabilization gate, revalidates the exact head SHA, and uses squash merge with branch deletion. Do not fall back to merge commits or rebase when squash is unavailable.
