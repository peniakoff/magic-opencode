...
3. Inspect repository instructions, contribution docs, manifests, lockfiles, CI workflows, current branch, remote default branch, status, and relevant recent history. Use `index_status` and semantic codebase tools for conceptual discovery when a usable index exists; otherwise fall back immediately to LSP and targeted grep/read.
...
Give one `implementer` a consolidated edit-only brief with the acceptance criteria, allowed scope, relevant evidence, required tests, documentation, versioning, and validation commands to return. This is mandatory when the change touches dependencies, manifests, lockfiles, migrations, generated registries, central exports, schemas, shared contracts, or when write-scope independence is uncertain.

### Parallel mode — only for two proven-independent units

Use at most two `implementer` instances and only when all conditions in the orchestrator's parallel implementation policy are satisfied.

1. Define lane `a` and lane `b` with explicit, non-overlapping file/directory scopes. Neither lane may depend on uncommitted output from the other or change a shared mutable contract.
...
1. Use repository-native runners and scripts. Never create ad hoc Python, Node.js, shell, or HTML scripts as substitutes for unit, integration, or end-to-end tests.
2. Durable browser regressions belong in the repository's established E2E framework. Use `browser-qa` through Playwright MCP only as supplementary exploratory evidence for changed user-facing web flows.
3. Add dependencies only when required. Dependency changes force sequential mode. Choose the latest stable version compatible with the repository runtime and lockfile, verifying unfamiliar APIs with Context7 or primary vendor documentation. Before any other feature edit, the implementer must update registry dependencies on the clean feature branch through `bash .opencode/scripts/dependency-update.sh <add|add-dev|remove> <npm|yarn|pnpm|bun> <package>...`, or its `batch` form. Immediately inspect and preliminarily review the resulting manifest and lockfile before continuing. Do not perform unrelated upgrades.
4. Update affected documentation. If the repository maintains a changelog and version manifest, follow its release policy. Unless repository rules say otherwise: feature -> minor, fix -> patch, breaking change at or above 1.0 -> major, and breaking change below 1.0 -> minor.
5. Run `bash .opencode/scripts/github-delivery.sh inspect` to obtain the actual root branch, status, complete changed-path set, and diff. Pass that exact evidence to `reviewer` for a preliminary review of package scripts, hooks, build configuration, test files, CI definitions, and other executable inputs. Route findings to the responsible implementer, inspect again, and repeat this preliminary gate after corrections.
6. Only after preliminary review have `test-debugger` or the orchestrator run checks in increasing cost order: focused tests, formatting check, lint/static analysis, type checking, broader tests, then build/package/synthesis checks required by repository instructions and CI.
...
1. After preliminary executable-input review and local validation, run the trusted root `inspect` action again and call `reviewer` with the acceptance criteria, actual combined diff and changed-path evidence, and exact command results for the independent final review.
2. Route every actionable finding back to an implementer. Use one normal implementer for integration-level fixes; reuse a parallel lane only when the repair again satisfies every parallel precondition.
...
4. Do not continue to delivery while actionable findings, unexplained failures, unreviewed generated changes, or active parallel worktrees remain.

## 6. Commit, PR, CI, and merge

1. Run the trusted root `inspect` action for the final diff and status. Pass its complete task-owned changed-path list to `bash .opencode/scripts/github-delivery.sh commit <conventional-message> <path>...`. Before staging, the wrapper requires the explicit list to match every working-tree change; it then starts from an empty index, rejects escaping and sensitive paths, checks the staged diff, and creates the Conventional Commit.
2. Run the wrapper's `push` action, then `create-pr <issue-url> <title> <body>`. The pull request body must contain the implementation summary, exact local validation results, risks or migrations, and `Closes #<issue-number>`.
...
4. Run `wait-checks <pr-number>`. Persistent zero checks, an unstable set, pending required review, requested changes, conflict, or skipped/cancelled/failed check blocks merge.
...
6. When the PR is mergeable, review is complete, and all checks pass, read and record the reviewed `headRefOid`, then run `bash .opencode/scripts/github-delivery.sh merge <pr-number> <head-sha>`. Do not fall back to merge commits or rebase when squash is unavailable.
...
3. Report the issue URL, PR URL, final commit on `main`, changed behavior, exact validation commands and results, GitHub Actions outcome, squash merge, issue state, parallel-lane usage if any, and local/remote branch cleanup.
