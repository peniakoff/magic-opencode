...
Use `test-debugger` for ambiguous failures, `browser-qa` for supplementary exploratory user-flow checks, `security-reviewer` for sensitive boundaries, and exactly one parent `reviewer` per review gate (brief includes inspect/diff; never `bugbot` or `explore`). The reviewer must treat malformed/invalid input that succeeds when the contract requires rejection as an actionable correctness defect, even if the produced value coincidentally matches a corrected input. Standards/protocol compliance claims must match the actual supported subset.
